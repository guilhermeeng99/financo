...
2. **Auth-first** — the cubit always emits `StartupLoading(progress: 0)` first, regardless of the current `AuthBloc` state, so the UI animates in consistently.
...
6. **Sync progress sentinel** — the cubit emits `StartupLoading(progress: 0.3)` immediately before invoking `fullSync`. The `0.3` literal is asserted by tests; the page derives the localized step label from this threshold (`progress >= 0.3` → "syncing data" copy, else "checking auth").
7. **Sync failure is terminal here** — any `Exception` from `fullSync` is logged for diagnostics (`dart:developer log`) and mapped to the field-less `StartupError`; the raw exception string is never surfaced — the page renders localized error copy. The user must retry; the cubit does not fall through to `StartupAuthenticated`.
...
StartupError   // field-less — the page owns the localized error copy
```

**Transitions:**

```
StartupInitial ──initialize()──→ StartupLoading(progress: 0)

StartupLoading(0)   ──auth = Unauthenticated──→ StartupUnauthenticated
                    ──auth = AuthError──────→ StartupUnauthenticated   [treated as not signed in]
                    ──auth = Authenticated──→ StartupLoading(progress: 0.3)

StartupLoading(0.3) ──fullSync ok─→ StartupAuthenticated(userId)
                    ──fullSync throw─→ StartupError

StartupError ──retry tap → initialize()──→ StartupLoading(0) → …
```
...
- The step label is derived from `progress` (`>= 0.3` → syncing copy, else checking-auth copy) — the cubit carries no user-facing strings.
...
- **`AccessDenied` arrives before auth resolves** — router redirect short-circuits before `StartupPage` finishes rendering; cubit may still be in `StartupLoading` when the page is torn down. Cubit `close()` is fine — `_waitForAuth` cancels its stream subscription (`await subscription.cancel()`) right after the completer resolves; there is no `finally` block.
...
- `t.startup.errorTitle` / `t.startup.errorBody` / `t.startup.errorRetry` — error block.

The cubit carries no strings at all — `StartupLoading` exposes only a `progress` double and `StartupError` is field-less — so every visible label (steps and error copy) is localized in the page layer.
...
- Loading sentinels: first emission is `StartupLoading(progress: 0)`; a `StartupLoading(progress: 0.3)` is emitted before completion.
