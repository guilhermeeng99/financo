...
| **Routing / shell** | ~~Keep Financo routes (`/investments`, `/investments/class/:id`); do **not** adopt Investanco's `/allocation` landing tab.~~ **Superseded (F7/F8 + allocation port):** routes are now `/investing/*` (`app_routes.dart`); the Investing sidebar group defaults to `/investing/allocation` and the class detail lives at `/investing/allocation/class/:id`. The session-scoped cubits still plug into the `ShellRoute` seam with `userId` from `AuthBloc.state` at mount (CLAUDE.md lifecycle). |
