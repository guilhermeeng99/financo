import {
  buildMessage,
  PendingTransaction,
} from '../src/transactions/notifyTransactionsDue';

// `buildMessage` is pure: it derives notification copy from a list of pending
// transactions with no I/O, so we test it directly (no firebase-admin mock needed).
...
describe('buildMessage', () => {
  ...
  describe('multiple transactions', () => {
    it('summarises when all transactions are due today', () => {
      const result = buildMessage([
        makeTransaction({ id: 'a', dueDate: laterToday() }),
        makeTransaction({ id: 'b', dueDate: startOfToday() }),
        makeTransaction({ id: 'c', dueDate: laterToday() }),
      ]);
    ...
    it('summarises a mix of overdue and due-today transactions', () => {
      const result = buildMessage([
        makeTransaction({ id: 'a', dueDate: yesterday() }),
        makeTransaction({ id: 'b', dueDate: yesterday() }),
        makeTransaction({ id: 'c', dueDate: laterToday() }),
      ]);
